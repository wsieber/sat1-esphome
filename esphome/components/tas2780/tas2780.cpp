...
static inline uint32_t volts_to_lim_q18(float v_peak) {
  if (!std::isfinite(v_peak) || v_peak < 0.0f)
    v_peak = 0.0f;
...
bool TAS2780::write_volume_() {
  ...
  */
  ...
  const uint8_t dvc = clamp<uint8_t>(static_cast<uint8_t>(std::lround(attn_db * 2.0f)), 0, 0xC8);

  ESP_LOGD(TAG, "Vol u=%0.3f range=[%0.3f..%0.3f] x=%0.3f gamma=%0.2f y=%0.3f attn=%5.2fdB DVC=0x%02X", u,
           this->vol_range_min_, this->vol_range_max_, x, kGamma, y, attn_db, dvc);
...
float TAS2780::calc_and_write_amp_level_(float v_cap_dBV) {
  ...
  const int code = static_cast<int>(std::clamp(steps_f, 0.0f, (kAmpMax_dBV - kAmpMin_dBV) / kStep_dB));
...
void TAS2780::configure_STL_(float th_max_vpk, bool enable, uint8_t max_attn_db, uint8_t attack_code, uint8_t hold_code,
                             uint8_t release_code, bool pause_during_bop, bool headroom_enable, float th_min_vpk,
                             float inf_pt_vpk) {
