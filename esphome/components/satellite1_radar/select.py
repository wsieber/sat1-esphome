...
CONF_LD2450_ZONE_TYPE = "ld2450_zone_type"
...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_LD2450_ZONE_TYPE): select.select_schema(
            Satellite1RadarSelect,
...
async def to_code(config):
    ...
    if CONF_LD2410_DISTANCE_RESOLUTION in config:
        s = await select.new_select(config[CONF_LD2410_DISTANCE_RESOLUTION], options=[])
    ...
    if CONF_LD2450_ZONE_TYPE in config:
        s = await select.new_select(config[CONF_LD2450_ZONE_TYPE], options=[])
        cg.add(hub.set_ld2450_zone_type_select(s))
